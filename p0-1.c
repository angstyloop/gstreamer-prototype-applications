/*
clear && gcc p0-1.c -o p0-1 `pkg-config --cflags --libs gtk+-3.0 gstreamer-video-1.0` && ./p0-1

BUGS

gst_video_frame_map_id: "assertion 'info->finfo->format == meta->format' failed"

Happens sometimes on click. Evidently, this can happen when the format
changes, and is not necessarily a big deal.

https://github.com/GStreamer/gst-plugins-base/blob/master/gst-libs/gst/video/video-frame.c
https://bugzilla.gnome.org/show_bug.cgi?id=763441&GoAheadAndLogIn=1

I think this is why the capsfilters were there originally, but they weren't working.
I removed the "capsfilters" since they didn't seem to be doing anything. They won't
be needed in the next prototype anyway since it's all audio.
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    gtk_widget_show_all(window);
      ...
      gst_element_link_many (src, q1, conv_before, effect, conv_after, q2, sink, NULL);
