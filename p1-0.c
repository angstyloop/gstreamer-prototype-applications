/*
gcc -g p1-0.c -o p1-0 `pkg-config --cflags --libs gtk+-3.0 gstreamer-video-1.0`
*/
...
activate(GtkApplication *app, gpointer _data/*gpointer data*/)
{
  ...
  for (unsigned i = 0; i < 4; ++i) {
    // Set the size and alignment of each button.
    gtk_overlay_add_overlay(GTK_OVERLAY(root_pane), buttons[i]);
    gtk_widget_set_size_request(buttons[i], 70, 30);
    ...
    // Simple positioning.
    // TODO: use the grid example to make this more better
    gtk_widget_set_halign(buttons[i], aligns[i][0]);
    gtk_widget_set_valign(buttons[i], aligns[i][1]);
  }

  gtk_widget_set_halign(buttons[0], GTK_ALIGN_START);
  gtk_widget_set_valign(buttons[0], GTK_ALIGN_START);
  gtk_widget_set_halign(buttons[1], GTK_ALIGN_START);
  gtk_widget_set_valign(buttons[1], GTK_ALIGN_END);
  gtk_widget_set_halign(buttons[2], GTK_ALIGN_END);
  gtk_widget_set_valign(buttons[2], GTK_ALIGN_START);
  gtk_widget_set_halign(buttons[3], GTK_ALIGN_END);
  gtk_widget_set_valign(buttons[3], GTK_ALIGN_END);
