/*
gcc `pkg-config --cflags gtk+-3.0` -o four-buttons-grid four_buttons_grid.c `pkg-config --libs gtk+-3.0`
*/

#include <gtk/gtk.h>

static void print_label(GtkWidget* widget, gpointer data) {
...
static void activate(GtkApplication* app, gpointer user_data) {
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(print_label), NULL);
    ...
    // Place the third button in the grid cell (0, 1) without spanning.
    ...
    // Place the fourth button in the grid cell (1, 1) without spanning.
    gtk_grid_attach(GTK_GRID(grid), button, 1, 1, 1, 1);

    // Create a fifth button, the exit button.
    button = gtk_button_new_with_label("exit");
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(gtk_widget_destroy), window);

    // Place the fifth button in the grid cell (0, 2), spanning two columns.
